...
pub(crate) fn input_pipe() -> (Pipe, WritableStream) {
    ...
    let qs = web_sys::QueuingStrategy::new();
    qs.set_high_water_mark(256.0);
    qs.set_size(callback.into_js_value().unchecked_ref());

    let stream =
        WritableStream::new_with_underlying_sink_and_strategy(sink.unchecked_ref(), &qs).unwrap();
...
pub(crate) fn output_pipe() -> (Pipe, ReadableStream) {
    ...
    let stream =
        ReadableStream::new_with_underlying_source_and_strategy(source.unchecked_ref(), &qs)
            .unwrap();
